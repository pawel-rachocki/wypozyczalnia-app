...
@Component({
  ...
  styleUrl: './klient-form.component.css'
})
export class KlientFormComponent implements OnInit {
  ...
  constructor(
    ...
    this.klientForm = this.fb.group({
      imie: ['', [Validators.required, Validators.minLength(2), Validators.maxLength(50)]],
      nazwisko: ['', [Validators.required, Validators.minLength(2), Validators.maxLength(50)]],
  ...
  onSubmit(): void {
    if (this.klientForm.valid) {
      ...
      operation.subscribe({
        ...
        error: (error) => {
          this.error = 'Błąd podczas zapisywania klienta';
          this.loading = false;
          console.error('Error saving klient:', error);
        }
      });
    } else {
